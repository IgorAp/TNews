...
function insereEstados(){
    estados.forEach(element => {
        var option = new Option(element.nome, element.nome);
...
function insereCidade(){
    ...
    xhttp.onreadystatechange = ()=>{
        let dados = JSON.parse(xhttp.response||"[]");
        let select = document.getElementById("cidade");
        select.innerHTML = '';
        select.add(new Option("Selecionar cidade",""));
        dados.forEach((element)=>{
            const option = new Option(element.nome, element.nome);
            select.add(option);
        });
    }
    xhttp.open("GET","https://servicodados.ibge.gov.br/api/v1/localidades/estados/"+id+"/municipios",true);
    xhttp.send(null);
